            ...
            with self.pause_cond:
                ...
                try:
                    if self.is_training==1:
                        # if it is training
                        if len(settings.bunch_asl_slices)==0 and len(settings.bunch_pet_slices)==0 and len(settings.bunch_t1_slices)==0:
                        ...
                        elif not len(settings.bunch_asl_slices)==0 and not len(settings.bunch_pet_slices)==0 and not len(settings.bunch_t1_slices)==0:
                            #if it is validating
                            if len(settings.bunch_asl_slices2) and len(settings.bunch_pet_slices2)and len(settings.bunch_t1_slices2):
                                settings.train_queue.acquire()
                                settings.bunch_asl_slices = np.vstack((settings.bunch_asl_slices,settings.bunch_asl_slices2))
                                settings.bunch_pet_slices = np.vstack((settings.bunch_pet_slices,settings.bunch_pet_slices2))
                                settings.bunch_t1_slices = np.vstack((settings.bunch_t1_slices,settings.bunch_t1_slices2))
                                settings.bunch_asl_slices2=[]
                                settings.bunch_pet_slices2=[]
                                settings.bunch_t1_slices2 = []
                                settings.train_queue.release()
                        ...
                        if ((len(settings.bunch_pet_slices_vl) == 0) \
                            ...
                            print('settings.bunch_asl_slices_vl lEN: %d' %(len(settings.bunch_asl_slices_vl )))
                                ...
                                &(len(settings.bunch_pet_slices_vl2) > 0)\
                                &(len(settings.bunch_t1_slices_vl2) > 0)):
                            settings.bunch_asl_slices_vl = np.vstack((settings.bunch_asl_slices_vl,settings.bunch_asl_slices_vl2))
                            settings.bunch_asl_slices_vl2 = []

                            settings.bunch_pet_slices_vl = np.vstack((settings.bunch_pet_slices_vl,settings.bunch_pet_slices_vl2))
                            settings.bunch_pet_slices_vl2 = []
