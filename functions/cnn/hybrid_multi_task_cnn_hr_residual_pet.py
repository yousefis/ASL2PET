...
class multi_stage_densenet:
    ...
    def level_design_nonloop(self, input, level_name, filters1, filters2, is_training, kernel_size, in_size, crop_size,
                     padding1, padding2, flag=2, paddingfree_scope='', filters3=0):
        with tf.variable_scope(level_name):
            conv1 = tf.layers.conv2d(input,
                                     filters=filters1,
                                     kernel_size=kernel_size,
    ...
    def encoder(self,encoder_tag,stack_concat,is_training,in_size3,in_size4,in_size5,crop_size0,crop_size1,crop_size2,config):
        # == == == == == == == == == == == == == == == == == == == == == ==
        # encoder #1
        # level 1 of unet
        [level_ds1, crop1] = self.dense_loop(stack_concat,
                                             encoder_tag +'level_ds1',
                                             filters1=16,
                                             filters2=16,
                                             filters3=32,
        ...
        with tf.variable_scope(encoder_tag+'maxpool2'):
            pool2 = tf.layers.max_pooling2d(inputs=level_ds2, pool_size=(2, 2), strides=(2, 2))

        # level 3 of unet
        [level_ds3, crop3] = self.dense_loop(pool2, encoder_tag+'level_ds3',
                                             filters1=64,
                                             filters2=64,
                                             filters3=128,
                                             is_training=is_training,
        ...
        return level_ds3, crop1, crop2

    def asl_decoder(self,decoder_tag,level_ds3,in_size0,crop_size0,is_training,config):
        with tf.variable_scope(decoder_tag+'conv_transpose1_fork2'):
            ...
            # level 2 of unet
        [level_us2, crop0] = self.dense_loop(deconv1, decoder_tag+'level_us2_fork2',
                                             filters1=64,
                                             filters2=32,
                                             is_training=is_training,
                                             kernel_size=3,
        ...
        with tf.variable_scope(decoder_tag+'conv_transpose2_fork2'):
        ...
        # level 1 of unet
        [level_us3, crop0] = self.dense_loop(deconv2, decoder_tag+'level_us3_fork2',
                                             filters1=32,
                                             filters2=32,
                                             is_training=is_training,
                                             kernel_size=3,
        ...
        with tf.variable_scope(decoder_tag+'last_layer_fork2'):
        ...
        return asl
    #================================================
    def pet_decoder(self,decoder_tag,in_size0,crop_size0,level_ds3,crop1,crop2, is_training, config):
        with tf.variable_scope(decoder_tag+'conv_transpose1'):
            deconv1 = tf.layers.conv2d_transpose(level_ds3,
                                                 filters=64,
                                                 kernel_size=3,
                                                 strides=(2, 2),
        ...
        with tf.variable_scope(decoder_tag+'concat1'):
            conc12 = tf.concat([crop2, deconv1], -1)

        # level 2 of unet
        [level_us2, crop0] = self.dense_loop(conc12, decoder_tag+'level_us2',
                                               filters1=64,
                                               filters2=32,
                                               is_training=is_training,
                                               kernel_size=3,
        ...
        with tf.variable_scope(decoder_tag+'conv_transpose2'):
            deconv2 = tf.layers.conv2d_transpose(level_us2,
                                                 filters=16,
                                                 kernel_size=3,
                                                 strides=(2, 2),
        ...
        with tf.variable_scope(decoder_tag+'concat2'):
            conc23 = tf.concat([crop1, deconv2], -1)

        # level 1 of unet
        [level_us3, crop0] = self.dense_loop(conc23, decoder_tag+'level_us3',
                                               filters1=32,
                                               filters2=32,
                                               is_training=is_training,
                                               kernel_size=3,
        ...
        with tf.variable_scope(decoder_tag+'last_layer'):
            conv1 = tf.layers.conv2d(level_us3,
                                     filters=8,
                                     kernel_size=1,
                                     padding='same',
    ...
    def multi_stage_densenet(self, asl_img, t1_img,pet_img,  input_dim, is_training,config,hybrid_training_flag,conv_transpose=True):
        ...
        [level_ds3, crop1, crop2]=self.encoder( 'encoder1_', stack_concat, is_training, in_size3, in_size4,
                                                in_size5, crop_size0, crop_size1, crop_size2, config)

        ####################################### PET Fork
        out_dim = 187#219
        pet1=self.pet_decoder('pet_decoder1_',in_size0,crop_size0,level_ds3,crop1,crop2, is_training, config)
        in_y = tf.keras.layers.ZeroPadding2D(padding=int((input_dim - out_dim) / 2))(pet1)
        [level_ds3_encoder2, crop1_encoder2, crop2_encoder2] = self.encoder('encoder2_', in_y, is_training, in_size3,
                                                                            in_size4,in_size5, crop_size0, crop_size1,
                                                                            crop_size2, config=[1,2,3,2,1])
        pet2 = pet1+ self.pet_decoder('pet_decoder2_', in_size0, crop_size0, level_ds3+level_ds3_encoder2,
                                      crop1_encoder2,crop2_encoder2, is_training, config=[1,2,3,2,1])

        ####################################### ASL Fork
        asl= self.asl_decoder( 'decoder1_',level_ds3, in_size0, crop_size0, is_training, config)
        # == == == == == == == == == == == == == == == == == == == == == ==
        # out_dim=219
        # in_y=tf.keras.layers.ZeroPadding2D(padding=int((input_dim-out_dim)/2))(y)
        # [level_ds3_encoder2, crop1_encoder2, crop2_encoder2] = self.encoder('encoder2_', in_y, is_training, in_size3, in_size4,
        #                                          in_size5, crop_size0, crop_size1, crop_size2, config)
        # asl2 = asl1+self.asl_decoder('decoder2_',level_ds3+level_ds3_encoder2, in_size0, crop_size0, is_training, config)
