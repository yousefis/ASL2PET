...
class multi_stage_densenet:
    ...
    def multi_stage_densenet(self, asl_img, t1_img,pet_img,  input_dim, is_training,config,hybrid_training_flag,conv_transpose=True):
        ...
        # T1 encoder
        # level 1 of unet
        [level_ds1_t1, crop1_t1] = self.dense_loop(t1_img,
                                                   'level_ds1_t1',
                                                   filters1=8,
                                                   filters2=8,
                                                   filters3=16,
        ...
        channel_attention1= self.channel_attention( name='level_ds1_t1', input=level_ds1_t1)


        with tf.variable_scope('maxpool1_t1'):
            pool1_t1 = tf.layers.max_pooling2d(inputs=level_ds1_t1, pool_size=(2, 2), strides=(2, 2))
        # level 2 of unet
        ...
        channel_attention2 = self.channel_attention(name='level_ds1_t2', input=level_ds2_t1)
        with tf.variable_scope('maxpool2_t1'):
            pool2_t1 = tf.layers.max_pooling2d(inputs=level_ds2_t1, pool_size=(2, 2), strides=(2, 2))

        # level 3 of unet
        [level_ds3_t1, crop3] = self.dense_loop(pool2_t1, 'level_ds3_t1',
                                                filters1=32,
                                                filters2=32,
                                                filters3=64,
                                                is_training=is_training,
        ...
        channel_attention3 = self.channel_attention(name='level_ds1_t3', input=level_ds3_t1)
        #== == == == == == == == == == == == == == == == == == == == == ==
        #ASL encoder
        #level 1 of unet
        [level_ds1_asl, crop1_asl] = self.dense_loop(asl_img,
                                               ...
                                               padding2='same',
                                               flag=1,loop=config[0])
        soft_skip1 = tf.concat([channel_attention1, level_ds1_asl],-1)
        with tf.variable_scope('maxpool1_asl'):
            pool1_asl = tf.layers.max_pooling2d(inputs=soft_skip1, pool_size=(2, 2), strides=(2, 2))
        # level 2 of unet
        [level_ds2_asl, crop2_asl] = self.dense_loop(pool1_asl,
                                               ...
                                               padding2='same',
                                               flag=1,loop=config[1])
        soft_skip2 = tf.concat([channel_attention2, level_ds2_asl],-1)
        with tf.variable_scope('maxpool2_asl'):
            pool2_asl = tf.layers.max_pooling2d(inputs=soft_skip2, pool_size=(2, 2), strides=(2, 2))

        # level 3 of unet
        [level_ds3_asl, crop3_asl] = self.dense_loop(pool2_asl, 'level_ds3_asl',
                                               filters1=32,
                                               filters2=32,
                                               filters3=64,
                                               is_training=is_training,
                                               ...
                                               padding2='same',
                                               flag=1,loop=config[2])
        soft_skip3 = tf.concat([channel_attention3, level_ds3_asl],-1)
        ...
        with tf.variable_scope('concat1'):
            conc12 = tf.concat([crop2_asl, deconv1], -1)
        ...
        with tf.variable_scope('concat2'):
            conc23 = tf.concat([crop1_asl, deconv2], -1)
