...
class multi_stage_densenet:
    ...
    def multi_stage_densenet(self, asl_img, t1_img,pet_img,  input_dim, is_training,config,hybrid_training_flag,residual_attention_map,conv_transpose=True):
        ...
        with tf.variable_scope('stack-contact'):
            ...
            stack_concat = tf.concat([stack_concat, asl_img1], -1)
            stack_concat = tf.concat([stack_concat, t1_img1], -1)
        ...
        with tf.variable_scope('conv_transpose1'):
            deconv1 = tf.layers.conv2d_transpose(level_ds3,
                                                 ...
                                                 padding='valid',
                                                 use_bias=False)
        skip_channel_attention1= self.channel_attention( name='skip_channel_attention1', input=crop2)

        with tf.variable_scope('concat1'):
            conc12 = tf.concat([skip_channel_attention1, deconv1], -1)
        ...
        with tf.variable_scope('conv_transpose2'):
            deconv2 = tf.layers.conv2d_transpose(level_us2,
                                                 ...
                                                 padding='valid',
                                                 use_bias=False)
        skip_channel_attention2 = self.channel_attention(name='skip_channel_attention1', input=crop1)

        with tf.variable_scope('concat2'):
            conc23 = tf.concat([skip_channel_attention2, deconv2], -1)
