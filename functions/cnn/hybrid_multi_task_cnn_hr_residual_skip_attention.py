...
class multi_stage_densenet:
    ...
    def spatial_channel_attention(self,name,input,is_training):
        with tf.name_scope(name+'_channel_attention'):
            g_max_pool1 =tf.keras.layers.GlobalMaxPool2D(data_format='channels_last')(input)
            ...
            result1=tf.multiply(tf.expand_dims(tf.expand_dims(sigmoid1,1),1)   ,input)
        with tf.name_scope(name + '_spatial_attention'):
            ave_max_pool1 = tf.concat([tf.expand_dims(tf.reduce_mean(result1,axis=-1),-1),tf.expand_dims(tf.reduce_max(result1,-1),-1)],-1)
            ave_max_pool_conv = tf.layers.conv2d(ave_max_pool1,
                                        filters=1,
                                        kernel_size=3,
                                        padding='same',
                                        activation=None,
                                        dilation_rate=1)
            bn3 = tf.layers.batch_normalization(ave_max_pool_conv, training=is_training, renorm=False)
            bn3 = tf.nn.leaky_relu(bn3)
            sigmoid2 = tf.nn.sigmoid(bn3)
            result2 = tf.multiply(sigmoid2, result1)

            #bottelneck layer:
            # ave_max_pool_conv = tf.layers.conv2d(result2,
            #                                      filters=int(result2.get_shape()[-1].value),
            #                                      kernel_size=1,
            #                                      padding='same',
            #                                      activation=None,
            #                                      dilation_rate=1)
            # bn4 = tf.layers.batch_normalization(ave_max_pool_conv, training=is_training, renorm=False)
            # bn4 = tf.nn.leaky_relu(bn4)
        return result2


    #==================================================
    def multi_stage_densenet(self, asl_img, t1_img,pet_img,  input_dim, is_training,config,hybrid_training_flag,residual_attention_map,conv_transpose=True):
        # mri=None
        ...
        #multiply by attention map
        asl_img1 =tf.multiply(asl_img ,residual_attention_map)
        t1_img1 =tf.multiply(t1_img,residual_attention_map)

        img_rows=[]
        img_rows.append(asl_img1)
        img_rows.append(t1_img1)
        ...
        skip_attention1 = self.spatial_channel_attention(name='skip_attention1', input=level_us2, is_training=is_training)
        ...
        skip_attention2 = self.spatial_channel_attention(name='skip_attention2', input=level_us3,
                                                         is_training=is_training)
        ...
        with tf.variable_scope('attention_block'):
            subtract = asl_img[:,
                       tf.to_int32(input_dim / 2) - tf.to_int32((crop_size2 - 1) / 2) - 1:
                       tf.to_int32(input_dim / 2) + tf.to_int32((crop_size2 - 1) / 2),
                       tf.to_int32(input_dim / 2) - tf.to_int32((crop_size2 - 1) / 2) - 1:
                       tf.to_int32(input_dim / 2) + tf.to_int32((crop_size2 - 1) / 2), :] - asl
            ave_max_pool1 = tf.concat([tf.expand_dims(tf.reduce_mean(subtract, axis=-1), -1),
                                       tf.expand_dims(tf.reduce_max(subtract, axis=-1), -1)], 3)
            ave_max_pool_conv = tf.layers.conv2d(ave_max_pool1,
                                                 filters=1,
                                                 kernel_size=3,
                                                 padding='same',
            ...
            bn3 = tf.layers.batch_normalization(ave_max_pool_conv, training=is_training, renorm=False)
            bn3 = tf.nn.leaky_relu(bn3)
            new_att_map = tf.nn.sigmoid(bn3)
            # asl_att = tf.multiply(sigmoid2, asl_img)
            # t1_att = tf.multiply(sigmoid2, t1_img)
        ...
        [level_us2, crop0] = self.dense_loop(conc12, 'level_us2',
                                               ...
                                               crop_size=crop_size0,
                                               padding1='same', padding2='same',loop=config[3])
        skip_conc1 = tf.concat([skip_attention1, level_us2], -1)
        with tf.variable_scope('conv_transpose2'):
            deconv2 = tf.layers.conv2d_transpose(skip_conc1,
                                                 filters=16,
                                                 kernel_size=3,
                                                 strides=(2, 2),
                                                 padding='valid',
        ...
        [level_us3, crop0] = self.dense_loop(conc23, 'level_us3',
                                               ...
                                               crop_size=crop_size0,
                                               padding1='same', padding2='same',loop=config[4])
        skip_conc2 = tf.concat([skip_attention2, level_us3], -1)

        with tf.variable_scope('last_layer'):
            conv1 = tf.layers.conv2d(skip_conc2,
                                     filters=8,
                                     kernel_size=1,
                                     padding='same',
                                     activation=None,
