...
class multi_stage_densenet:
    ...
    def channel_attention(self,name,input):
        with tf.name_scope(name+'_channel_attention'):
            ...
            channel_attention=tf.multiply(tf.expand_dims(tf.expand_dims(sigmoid1,1),1)  ,input)
    ...
    def multi_stage_densenet(self, asl_img, t1_img,pet_img,  input_dim, is_training,config,hybrid_training_flag,conv_transpose=True):
        ...
        # asl encoder
        # level 1 of unet
        [level_ds1_asl, crop1_asl] = self.dense_loop(asl_img,
                                                   'level_ds1_asl',
                                                   filters1=8,
                                                   filters2=8,
                                                   filters3=16,
        ...
        channel_attention1= self.channel_attention( name='level_ds1_asl', input=level_ds1_asl)


        with tf.variable_scope('maxpool1_asl'):
            pool1_asl = tf.layers.max_pooling2d(inputs=level_ds1_asl, pool_size=(2, 2), strides=(2, 2))
        # level 2 of unet
        [level_ds2_asl, crop2_asl] = self.dense_loop(pool1_asl,
                                                   'level_ds2_asl',
                                                   filters1=16,
                                                   filters2=16,
                                                   filters3=32,
        ...
        channel_attention2 = self.channel_attention(name='level_ds1_t2', input=level_ds2_asl)
        with tf.variable_scope('maxpool2_asl'):
            pool2_asl = tf.layers.max_pooling2d(inputs=level_ds2_asl, pool_size=(2, 2), strides=(2, 2))

        # level 3 of unet
        [level_ds3_asl, crop3] = self.dense_loop(pool2_asl, 'level_ds3_asl',
                                                filters1=32,
                                                filters2=32,
                                                filters3=64,
                                                is_training=is_training,
        ...
        channel_attention3 = self.channel_attention(name='level_ds1_t3', input=level_ds3_asl)
        #== == == == == == == == == == == == == == == == == == == == == ==
        #t1 encoder
        #level 1 of unet
        [level_ds1_t1, crop1_t1] = self.dense_loop(asl_img,
                                               'level_ds1_t1',
                                               filters1=8,
                                               filters2=8,
                                               filters3=16,
                                               ...
                                               padding2='same',
                                               flag=1,loop=config[0])
        soft_skip1 = tf.concat([channel_attention1, level_ds1_t1],-1)
        with tf.variable_scope('maxpool1_t1'):
            pool1_t1 = tf.layers.max_pooling2d(inputs=soft_skip1, pool_size=(2, 2), strides=(2, 2))
        # level 2 of unet
        [level_ds2_t1, crop2_t1] = self.dense_loop(pool1_t1,
                                               'level_ds2_t1',
                                               filters1=16,
                                               filters2=16,
                                               filters3=32,
                                               ...
                                               padding2='same',
                                               flag=1,loop=config[1])
        soft_skip2 = tf.concat([channel_attention2, level_ds2_t1],-1)
        with tf.variable_scope('maxpool2_t1'):
            pool2_t1 = tf.layers.max_pooling2d(inputs=soft_skip2, pool_size=(2, 2), strides=(2, 2))

        # level 3 of unet
        [level_ds3_t1, crop3_t1] = self.dense_loop(pool2_t1, 'level_ds3_t1',
                                               filters1=32,
                                               filters2=32,
                                               filters3=64,
                                               is_training=is_training,
                                               ...
                                               padding2='same',
                                               flag=1,loop=config[2])
        soft_skip3 = tf.concat([channel_attention3, level_ds3_t1],-1)

        ####################################### PET Fork
        with tf.variable_scope('conv_transpose1'):
            deconv1 = tf.layers.conv2d_transpose(soft_skip3,
                                                 filters=64,
                                                 kernel_size=3,
                                                 strides=(2, 2),
                                                 padding='valid',
        ...
        with tf.variable_scope('concat1'):
            conc12 = tf.concat([crop2_t1, deconv1], -1)
        ...
        with tf.variable_scope('concat2'):
            conc23 = tf.concat([crop1_t1, deconv2], -1)
        ...
        with tf.variable_scope('conv_transpose1_fork2'):
            deconv1 = tf.layers.conv2d_transpose(level_ds3_asl,
                                                 filters=64,
                                                 kernel_size=3,
                                                 strides=(2, 2),
                                                 padding='valid',
