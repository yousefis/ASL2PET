...
from functions.cnn.hybrid_single_task_cnn_hr_not1_no_residual_att import multi_stage_densenet
...
def print_ave_std(ssim_hc,ssim_nc,psnr_hc,psnr_nc,mse_hc,mse_nc):
    ...
    print(np.average(np.array(psnr_hc)).round(2), np.std(np.array(psnr_hc)).round(2))
    ...
    print(np.average(np.array(psnr_nc)).round(2), np.std(np.array(psnr_nc)).round(2))
    print(np.average(np.array(mse_nc)).round(2), np.std(np.array(mse_nc)).round(2))

    print('All:')
    print(np.average(np.array(ssim_hc.append(ssim_nc))).round(2), np.std(np.array(ssim_hc.append(ssim_nc))).round(2))
    print(np.average(np.array(mse_hc.append(mse_nc))).round(3), np.std(np.array(mse_hc.append(mse_nc))).round(3))
    print(np.average(np.array(psnr_hc.append(psnr_nc))).round(2), np.std(np.array(psnr_hc.append(psnr_nc))).round(2))
...
def test_all_nets(out_dir, Log, which_data,fold):
    ...
    pet_y = msdensnet.multi_stage_densenet(asl_img=asl_plchld,
                                                               t1_img=t1_plchld,
                                                               pet_img=pet_plchld,
    ...
    with tf.name_scope('cost'):
        ...
        cost_withpet = tf.reduce_mean( loss_pet)
    ...
    _meas = _measure()
    copyfile('./test_single_task_not1_nresdatt.py',
             parent_path + Log + out_dir + 'test_single_task_not1_nresdatt.py')
    ...
    for scan in range(len(data)):
        ...
        for img_indx in range(np.shape(imm[3])[0]):
            ...
            [loss, psnr1, mse1, pet_out] = sess.run([ssim_pet, psnr, mse, pet_y],
