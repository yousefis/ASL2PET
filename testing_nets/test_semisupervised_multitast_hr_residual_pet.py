...
# asl_size =231
# pet_size = 219
asl_size = 199  # 231  # input slice size
pet_size = 187  # 219  # output slice size
...
def test_all_nets(out_dir, Log, which_data):
    ...
    for scan in range(len(data)):
        ...
        for img_indx in range(np.shape(imm[3])[0]):
            ...
            [loss,pet_out1,pet_out2,asl_out] = sess.run([ssim_pet,pet_y1,pet_y2,asl_y],
            ...
            matplotlib.image.imsave(parent_path + Log + out_dir + ss[-3] + '/' + ss[-1].split(".")[0].split("ASL_")[
                1] + '/res_asl' + name + '_' + str(ssim) + '.png', np.squeeze(pet_out2), cmap='gray')
            matplotlib.image.imsave(parent_path + Log + out_dir + ss[-3] + '/' + ss[-1].split(".")[0].split("ASL_")[
                1] + '/res_pet' + name + '_' + str(ssim) + '.png', np.squeeze(asl_out), cmap='gray')
...
if __name__ == "__main__":
    Log = "Log_asl_pet/denseunet_hybrid_hr_residual_pet_01/"
    which_data = 1
    if which_data == 1:
