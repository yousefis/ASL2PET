...
from functions.losses.ssim_loss import SSIM,PSNR,MSE
# from functions.reader.data_reader_hybrid_hr_cross_validate import *
...
def test_all_nets(out_dir, Log, which_data):
    ...
    asl_y, pet_y, new_att_map = msdensnet.multi_stage_densenet(asl_img=asl_plchld,
                                                               t1_img=t1_plchld,
                                                               pet_img=pet_plchld,
    ...
    _meas = _measure()
    copyfile('./test_semisupervised_multitast_hr_rest_residual_skip_attention.py',
             parent_path + Log + out_dir + 'test_semisupervised_multitast_hr_rest_residual_skip_attention.py')
    ...
    for scan in range(len(data)):
        ...
        for img_indx in range(np.shape(imm[3])[0]):
            ...
            print(list_name[img_indx], ': ', ssim,',PSNR: ',psnr1,',MSE: ',mse1)
    ...
    a = {'SSIM_HC': list_ssim_HC, 'SSIM_NC': list_ssim_NC,
         'MSE_HC': list_mse_HC, 'MSE_NC': list_mse_NC,
         'PSNR_HC': list_psnr_HC, 'PSNR_NC': list_psnr_NC}
...
if __name__ == "__main__":
    Log = "Log_asl_pet/rest/residual_skip_attention/"
    # Log = "Log_asl_pet/rest/residual_attention2/"
    # Log = "Log_asl_pet/rest/cross_validation/residual_resid_skip_attention_02/"
    which_data = 1
    if which_data == 1:
