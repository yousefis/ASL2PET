...
def test_all_nets(out_dir, Log, which_data):
    ...
    _meas = _measure()
    copyfile('./test_semisupervised_multitast_hr_rest_residual_pet.py',
             parent_path + Log + out_dir + 'test_semisupervised_multitast_hr_rest_residual_pet.py')
    ...
    for scan in range(len(data)):
        ...
        for img_indx in range(np.shape(imm[3])[0]):
            ...
            [loss,pet_out,asl_out] = sess.run([ssim_pet,pet_y,asl_y2],
            ...
            sitk.WriteImage( sitk.GetImageFromArray(np.squeeze(pet_out)),nm_fig + '/res_pet' + name + '_' + str(ssim) + '.mha')
            sitk.WriteImage( sitk.GetImageFromArray(np.squeeze(asl_out)),nm_fig + '/res_asl' + name + '_' + str(ssim) + '.mha')
...
if __name__ == "__main__":
    Log = "Log_asl_pet/rest/denseunet_hybrid_hr_residual_pet_01/"
    which_data = 2
    if which_data == 1:
