...
class multi_stage_densenet:
    ...
    def rotate_input(self,img_rows,is_training):
        ...
        with tf.variable_scope("Rotate"):
            ...
            degree_angle = tf.random_uniform([1],minval=-30, maxval=30, seed=self.seed())[0]
    ...
    def multi_stage_densenet(self, asl_img, t1_img,pet_img,  input_dim, is_training,conv_transpose=True):
        ...
        with tf.variable_scope('conv_transpose1'):
            deconv1 = tf.layers.conv2d_transpose(level_ds3,
                                                 ...
                                                 padding='valid',
                                                 use_bias=False)
            deconv1 = tf.layers.batch_normalization(deconv1, training=is_training, renorm=False)
        ...
        with tf.variable_scope('conv_transpose2'):
            deconv2 = tf.layers.conv2d_transpose(level_us2,
                                                 ...
                                                 padding='valid',
                                                 use_bias=False)
            deconv2 = tf.layers.batch_normalization(deconv2, training=is_training, renorm=False)
        ...
        with tf.variable_scope('last_layer'):
            ...
            y = tf.layers.batch_normalization(y, training=is_training, renorm=False)
            y = tf.nn.relu(y)
